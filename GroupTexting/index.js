...
const SendMessageToGroup = (event) => {
    ...
    changedArray.forEach(async (data, i) => {
        if (data) {
            ...
            formData.append('User', "babbo20")
            formData.append('Password', "@Online101")
            formData.append('PhoneNumbers', `${data}`)
            formData.append('Message', `${message.value}`)
            await fetch("https://cors-anywhere.herokuapp.com/"+"https://app.grouptexting.com/sending/messages?format=json", {
                method: "POST",
                headers: {
                    "Authorization": "Basic YmFiYm8yMDpAT25saW5lMTAx"
                },
                body: formData
            })
                .then(res => {
